"""
FM Synthesizer (DX7). 
The code mainly comes from https://github.com/fcaspe/ddx7 with minor adaptations.
"""
...
import numpy as np
from src.models.synths.synth_utils import upsample
from src.models.synths.synth_utils import Reverb
...
class FMsynth(nn.Module):
    def __init__(self,
        ...
        available_synths = {
            ...
            'fmguitar': fm_guitar_synth,
            'fmpiano': fm_piano_synth,
...
'''
Piano FM Synth - with phase wrapping (it does not change behaviour)
PATCH NAME: PIANO 1
OP3->OP2->OP1->|
  (R)OP6->OP4->|
  (R)OP6->OP5->|->out
'''
def fm_piano_synth(pitch, ol, fr, sample_rate, max_ol, use_safe_cumsum=False):    
        if(use_safe_cumsum==True):
            omega = cumsum_nd(2 * np.pi * pitch / sample_rate, 2*np.pi)
        else:
            omega = torch.cumsum(2 * np.pi * pitch / sample_rate, 1)
        ...
        # Torch unsqueeze with dim -1 adds a new dimension at the end of ol to match phases.
        op3_phase =  fr[OP3] * omega
        op3_output = torch.unsqueeze(ol[:,:,OP3], dim=-1) * torch.sin(op3_phase % (2*np.pi))
    
        op2_phase =  fr[OP2] * omega + 2 * np.pi * op3_output
        op2_output = torch.unsqueeze(ol[:,:,OP2], dim=-1) * torch.sin(op2_phase % (2*np.pi))
    
        op1_phase =  fr[OP1] * omega + 2 * np.pi * op2_output
        op1_output = torch.unsqueeze(ol[:,:,OP1], dim=-1) * torch.sin(op1_phase % (2*np.pi)) # output of stack of 3
    
        op6_phase =  fr[OP6] * omega
        op6_output = torch.unsqueeze(ol[:,:,OP6], dim=-1) * torch.sin(op6_phase % (2*np.pi))
    
        op4_phase =  fr[OP4] * omega + 2 * np.pi * op6_output
        op4_output = torch.unsqueeze(ol[:,:,OP4], dim=-1) * torch.sin(op4_phase % (2*np.pi)) # output of stack of 2*
    
        op5_phase =  fr[OP5] * omega + 2 * np.pi * op6_output
        op5_output = torch.unsqueeze(ol[:,:,OP5], dim=-1) * torch.sin(op5_phase % (2*np.pi)) # output of stack of 2* 
    
        return (op1_output + op4_output + op5_output)/max_ol

'''
Guitar FM Synth - with phase wrapping (it does not change behaviour)
PATCH NAME: GUITAR 1
OP6->OP5->|
  (R)OP4->|->OP3->|
        OP2->OP1->|->out     
'''
def fm_guitar_synth(pitch, ol, fr, sample_rate, max_ol, use_safe_cumsum=False):
    if(use_safe_cumsum==True):
        omega = cumsum_nd(2 * np.pi * pitch / sample_rate, 2*np.pi)
    else:
        omega = torch.cumsum(2 * np.pi * pitch / sample_rate, 1)
    ...
    op3_phase =  fr[OP3] * omega + 2 * np.pi * (op4_output +  op5_output)
    op3_output = torch.unsqueeze(ol[:,:,OP3], dim=-1) * torch.sin(op3_phase % (2*np.pi)) # output of stack of 2* 
    ...
    return (op1_output + op3_output)/max_ol
