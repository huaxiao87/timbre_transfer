...
class ProcessData():
    def __init__(self, silence_thresh_dB, sr, device, seq_len,
        ...
        self.time_shift_augmentation = time_shift_augmentation if time_shift_augmentation is not None else [0.]
    ...
    def process_indices(self, indices: list, time_augmentation: bool) -> list:
    # Length in samples.
        max_len = self.max_len * self.sr
        if time_augmentation:
            time_shifts = [int(self.sr * i) for i in self.time_shift_augmentation]
        else:
            time_shifts = [0]

        def expand_long(indices_tuple: tuple, time_shifts: list = time_shifts) -> list:
            seq = []
            for shift in time_shifts:
                starting_point = indices_tuple[0] + shift
                if indices_tuple[1] - starting_point > max_len:
                    # Add a time shift to the start of the sound.
                    ret = [(start, start+max_len) for start in np.arange(starting_point, indices_tuple[1] - max_len, max_len)]
                    ret.append((ret[-1][-1], min(ret[-1][-1] + max_len, indices_tuple[1])))
                    seq.extend(ret)
                else:
                    seq.append((starting_point, indices_tuple[1]))
            return seq
        new_indices = [*map(expand_long, indices)]
        new_indices = functools.reduce(operator.concat, new_indices, [])
        new_indices = [x for x in new_indices if (x[1] - x[0] > self.seq_len * self.sr)]
        new_indices = sorted(new_indices, key=lambda x: x[0])
        return new_indices
    ...
    def save_data(self, audio, f0, loudness, rms, h5f, counter, file_id):
        h5f.create_dataset(f'{counter}_audio', data=audio)
        h5f.create_dataset(f'{counter}_f0', data=f0)
        h5f.create_dataset(f'{counter}_loudness', data=loudness)
        h5f.create_dataset(f'{counter}_rms', data=rms)
        h5f.create_dataset(f'{counter}_file_id', data=file_id.encode())  # Store file_id as string
        return counter + 1
    ...
    def run_on_files(self, data_dir, input_dir, output_dir):
        ...
        counter = 0
        file_ids = []  # List to store file IDs for each segment

        for audio_file in tqdm(audio_files):
            file_id = audio_file.stem  # Use the file name without extension as the file ID
            if self.debug:
            ...
            if self.contiguous:
                ...
                sounds_indices = self.process_indices(sounds_indices, time_augmentation=True)
            if len(sounds_indices) == 0:
                continue

            for indices in sounds_indices:
                audio = data[indices[0]:indices[1]]
                # file_id = indices[2]  # Get the file ID
                if self.debug:
                ...
                # Further downsample the audio back to the other specified sample rates and return a dictionary.
                ...
                counter = self.save_data(audio, f0, loudness, rms, h5f, counter, file_id)
                file_ids.append(file_id)  # Append the file ID for each segment

        # Finished storing f0 and loudness
        self.close_h5(h5f)
        # Save file_ids in a json file
        with open(output_dir / f"{data_dir}_file_ids.json", "w") as f:
            json.dump(file_ids, f)
        return file_ids  # Return the list of file IDs
...
def create_mono_jazznet(instrument, audio_files, target_dir):
    target_dir = target_dir / instrument
    if not target_dir.exists():
        target_dir.mkdir()
    cur_audio_files = [audio_file for audio_file in audio_files]
...
def make_jazznet(args):
    CWD = Path(hydra.utils.get_original_cwd()) # Get current directory
    os.chdir(CWD)
    
    if args.jazznet is not None:
        jazznet_path = CWD / args.jazznet.source_folder

        if(args.skip_copy is False):

            # Create directories if needed
            dirs = args.jazznet.input_dir.split("/")
            target_dir = CWD
            for d in dirs:
                target_dir = target_dir / d
                #print(target_dir)
            ...
            # Find relevant audio files.
            # Concatenate two lists of files
            jazznet_audio_files = []
            if args.jazznet.patterns is not None:
                for pattern in args.jazznet.patterns:
                    jazznet_audio_files += list(jazznet_path.glob(f'./{pattern}/*/*.wav'))
            else:    
                raise Exception("No patterns defined for Jazznet dataset")
            

            print("[INFO] Jazznet Path: {}".format(jazznet_path))

            print("[INFO] Processing only monophonic files")
            
            print("[INFO] Number of files: {}".format(len(jazznet_audio_files)))

            # Partial function with instruments pre-configured for processing.
            create_mono_jazznet(instrument="piano", audio_files=jazznet_audio_files, target_dir=target_dir)
    ...
    if(args.skip_process is False):
        ...
        # Create output dirs if needed
        dirs = args.jazznet.output_dir.split("/")
        target_dir = CWD
        for d in dirs:
            target_dir = target_dir / d
            #print(target_dir)
        ...
        data_processor.run_on_dirs(CWD / args.jazznet.input_dir,
                                CWD / args.jazznet.output_dir)
    return
...
@hydra.main(version_base="1.3", config_path="./",config_name="create_data.yaml")
def main(args):

    if(args.process_testset is True): make_testset(args)
    
    if(args.process_jazznet is True): make_jazznet(args)
